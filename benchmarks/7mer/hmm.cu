...
#define NUM_OBS 101
#define NUM_PREDS 5
#define trans1_SIZE 65536
#define trans2_SIZE 16
#define outputProb_SIZE 1654784
#define initialProb_SIZE 262144
__device__
prob_t t_gamma;
__device__
prob_t t_trans1[65536];
__device__
prob_t t_trans2[16];
__device__
prob_t t_outputProb[1654784];
__device__
prob_t t_initialProb[262144];
__device__
...
prob_t output_prob(state_t x, obs_t o) {
  return (t_outputProb[((o * 16384) + (x % 16384))]);
}
__device__
prob_t transition_prob(state_t x, state_t y) {
  return ((t_trans1[(((x % 16384) * 4) + (y % 4))]) + (t_trans2[(y / 16384)]));
}
__device__
...
int forward_prob_predecessors(const prob_t (*alpha_prev), int instance, state_t state, prob_t (*probs)) {
  ...
  prob_t p;
  {
    ...
    while ((x1 < 4)) {
      {
        (pred = (((0 * 16384) + (x1 * 4096)) + ((state / 4) % 4096)));
  ...
  }
  {
    ...
    if (((state / 16384) == 14)) {
      {
        (pred = ((((state / 16384) + 1) * 16384) + (state % 16384)));
