...
impl SubVars<triton_ast::Expr> for triton_ast::Expr {
    fn sub_vars(self, env: &SubEnv<triton_ast::Expr>) -> triton_ast::Expr {
        match self {
            triton_ast::Expr::Var {id, ..} if env.contains_key(&id) => {
                env.get(&id).unwrap().clone()
            },
            _ => self.smap(|e| e.sub_vars(env))
        }
...
impl SubVars<triton_ast::Expr> for triton_ast::Stmt {
    fn sub_vars(self, env: &SubEnv<triton_ast::Expr>) -> triton_ast::Stmt {
        let s = self.smap(|s: triton_ast::Stmt| s.sub_vars(env));
        s.smap(|e: triton_ast::Expr| e.sub_vars(env))
    }
}
