...
fn convert_expr<'py>(expr : Bound<'py, PyAny>, env : &'py ConvertEnv<'py>) -> PyResult<ir::Expr> {
    if expr.is_instance(&env.ast.getattr("Name")?)? {
        ...
        Ok(ir::Expr::BinOp {lhs : Box::new(lhs), op, rhs : Box::new(rhs)})
        ...
        Ok(ir::Expr::ArrayAccess {target : Box::new(target), idx : Box::new(idx)})
    } else {
...
fn ir_elem_type<'py>(dtype : Bound<'py, PyAny>, id : &str) -> PyResult<ir::Type> {
...
fn ir_type<'py>(e : &Bound<'py, PyAny>, id : &str) -> PyResult<ir::Type> {
