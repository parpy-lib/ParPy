...
Parir can operate on any arrays or tensors, providing the `__cuda_array_interface__` attribute (e.g., Torch tensors allocated on the GPU and CuPy NDArrays), making it highly interoperable with existing code. Importantly, Parir aims not to replace such libraries but to complement them. Users can easily write specialized GPU kernels for algorithms not immediately supported by libraries to avoid the Python interpreter overhead and gain better control over parallelization.

The Parir compiler produces CUDA C++ code. The mapping from a Parir function to the CUDA C++ is intentionally relatively straightforward; a user should be able to identify how the for-loops of the original function are mapped to the GPU by looking at the CUDA C++ code. Advanced users can retrieve the CUDA C++ code and manually adjust it to use features inaccessible from Python (e.g., CUDA intrinsics or external libraries) and re-compile it using the Parir compiler (if the C API is left untouched).
...
When the `sum_rows` function is called, the Parir compiler generates GPU code based on the untyped Python-like representation from the first stage and the arguments provided to the function. The above call requests that any statements associated with the label `N` are mapped to `N` threads. The [Parallelization](#parallelization) section describes how the compiler maps these threads to the GPU grid.
...
```python
fn = parir.compile_string("sum_rows", code, includes=[], libs=[], extra_flags=[])
```

where the resulting function `fn` is a callable Python function expecting the same arguments as the original `sum_rows` function. The two required arguments are the function's name and the CUDA C++ code. The `includes` and `libs` keyword arguments specify include paths (`-I`) and library paths (`-L`) to be passed to the `nvcc` compiler (e.g. if the modified code depends on external libraries), and the `extra_flags` keyword argument provides other arguments to be immediately passed to `nvcc`.
