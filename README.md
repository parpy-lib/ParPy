...
The dependencies of Prickle can either be installed manually, or you can use the provided `Dockerfile`, which installs Prickle and includes all dependencies needed to run tests and benchmarks. As Prickle currently only generates CUDA code, you need an Nvidia GPU to be able to run the JIT-compiled code.
...
```bash
docker build --force-rm --tag=pyprickle .
docker run --rm -it --gpus all pyprickle
```

Alternatively, we can build the image and run it using `podman` as
```bash
podman build --force-rm --format=docker --tag=pyprickle .
podman run --rm -it --device "nvidia.com/gpu=all" pyprickle
```
...
The Prickle compiler is implemented in [Rust](https://www.rust-lang.org/), so it requires that the Rust compiler `rustc` is available on the path. Prickle also requires [Python](https://www.python.org/) and `pip` to be installed.

To be able to run the code generated by the Prickle compiler, you need to install [CUDA](https://developer.nvidia.com/cuda-downloads) and ensure that `nvcc` (the CUDA compiler) is in the `PATH`.

## Motivation

Prickle is a Python extension that performs configurable GPU parallelization by annotating for-loops. The key feature of Prickle is the ability to control not only what parts of a program to parallelize but also _how_ to parallelize.
